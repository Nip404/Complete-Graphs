...
def print_instructions():
    print("")
    print("SPACE to pause animation")
    print("ENTER to restart animation")
    print("Q to add 1 point")
    print("W to remove 1 point")
    print("A to add 10 points")
    print("S to remove 10 points")
    print("Z to add 50 points")
    print("X to remove 50 points")
   
def get_points(amount, radius, centre): 
    return [[[radius*math.cos(i) + centre[0], radius*math.sin(i) + centre[1]],\
        [(radius+20) * math.cos(i) + centre[0], (radius+20) * math.sin(i) + centre[1]], p] for p, i in enumerate([math.radians(e + 180 if e + 180 < 360 else e - 180) for e in [360 * (k/amount) for k in range(amount)]])]

def get_lines(points, factor): 
    return [[i, i * factor if i * factor < points else loop(i * factor, points)] for i in range(points)]
